...
class Bruter(object):
    ...
    def prune_browsers(self, browser) -> None:
        """Verilen tarayıcıyla aynı parolaya sahip tüm tarayıcıları kaldırın"""
        ...
        while self.is_alive:
            for pwd in self.password_manager.passlist:
                ...
                if not attack_started:
                    self.display.info("Şifre Saldırları Başlıyor. Bundan Sonrası Bende-ERKOYUNCU")
                    attack_started = True
    ...
    def start_daemon_threads(self):
        ...
        self.display.info("Proxyler Aranıyor.")

    def stop_daemon_threads(self):
        self.password_manager.stop()

    def start(self):
        self.display.info("Arka Planda Terör Estirmeye Başlıyorum Kontrol Bende -Erkoyuncu...")
